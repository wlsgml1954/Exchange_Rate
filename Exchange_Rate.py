...
def click_button(val):
    if val == "=":
        try:
            # 수식 계산
            result_value = eval(
                st.session_state.cal_formula.replace("x", "*").replace("%", "/100")
            )
...
def sync_formula():
    st.session_state.cal_formula = st.session_state.cal_input
...
user_input = st.text_input("수식입력", value=st.session_state.cal_formula)

if user_input != st.session_state.cal_formula:
    st.session_state.cal_formula = user_input
